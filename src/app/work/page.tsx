  ...
  return (
    <AppShell>
      <PageHeader
        eyebrow="Work Board"
        title="Operations"
        description="Direct the work. Assign ownership, set priorities, remove blockers, and inspect the evidence."
        action={
          <StatusBadge tone="gold">
            State updated{" "}
            {new Date(commandState.updatedAt).toISOString().slice(0, 10)}
          </StatusBadge>
        }
      />
      ...
      <DashboardCard
        title="Historical shipping ledger"
        eyebrow="Recorded verification · separate from order status"
      >
              ...
              <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                <div>
                  <div className="flex flex-wrap items-center gap-2">
                    <StatusBadge tone={statusTone(entry.evidenceStatus)}>
                ...
                <div className="rounded-md border border-white/10 bg-black/35 p-3">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {entry.changed.map((item) => (
                      <span
                        key={item}
                        className="inline-flex items-center gap-1 rounded bg-white/10 px-2 py-1 text-xs font-semibold text-[#d6d6d6]"
                      >
                        {item.startsWith("/") ? (
