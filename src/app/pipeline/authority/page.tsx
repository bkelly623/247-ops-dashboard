...
export default async function AuthorityPipelinePage() {
  ...
  const needsApproval = commandState.pipelines.authority.filter(
    (item) => item.approvalNeeded,
  ).length;
        ...
        {commandState.pipelines.authority.map((item) => (
          <DashboardCard key={item.id} title={item.target} eyebrow={item.type}>
            <div className="flex flex-wrap items-center gap-2">
              <StatusBadge
                tone={item.status.includes("Approval") ? "danger" : "warn"}
              >
                {item.status}
              </StatusBadge>
              <StatusBadge tone={riskTone(item.risk)}>
                {item.risk} risk
              </StatusBadge>
            </div>
            <div className="mt-5 space-y-3 text-sm leading-6 text-[#c9c9c9]">
              ...
              <div className="flex gap-3 rounded-md border border-white/10 bg-white/5 p-3">
                <ShieldAlert
                  className="mt-1 shrink-0 text-[#8ac9ba]"
                  size={18}
                />
                <p>
