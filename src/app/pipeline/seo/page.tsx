      ...
      <PageHeader
        ...
        action={
          <StatusBadge tone="gold">
            {commandState.pipelines.seo.length} tracked pages
          </StatusBadge>
        }
      />
      ...
      <DashboardCard title="Page Pipeline" eyebrow="Evidence before expansion">
        <div className="overflow-x-auto">
          <table className="w-full min-w-[920px] border-separate border-spacing-0 text-left text-sm">
            <thead className="text-xs uppercase tracking-[0.14em] text-[#8ac9ba]">
              <tr>
                ...
                <tr key={item.id} className="align-top">
                  <td className="border-b border-white/10 px-3 py-4 font-semibold text-white">
                    <span className="inline-flex items-center gap-2">
                      <ArrowUpRight size={14} className="text-[#8ac9ba]" />
                      {item.page}
                    </span>
                  </td>
                  ...
                  <td className="border-b border-white/10 px-3 py-4">
                    <StatusBadge
                      tone={item.status.includes("Needs") ? "warn" : "good"}
                    >
                      {item.status}
                    </StatusBadge>
                  </td>
